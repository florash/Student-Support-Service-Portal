...
This project can be deployed to GitHub Pages using the `docs` folder on the `main` branch.

1. Run `npm run build`.
2. Copy the contents of `dist` into a top-level `docs` folder.
3. Upload the project to GitHub.
4. In the repository, open `Settings` → `Pages`.
5. Under `Build and deployment`, choose `Deploy from a branch`.
6. Select `main` and `/docs`.

The project uses `HashRouter` and a relative Vite base path so internal routes work on GitHub Pages without extra server configuration.
